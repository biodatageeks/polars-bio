...
- Use `pyarrow>=23.0.1,<25` for all supported Python versions so PyArrow stays compatible with DataFusion Python 53 and Python 3.14 wheels while excluding vulnerable `pyarrow<23.0.1` releases.
...
- Arrow 58 Rust crates and PyArrow 23.0.1+ have different release cadences. FFI paths must be tested through Polars/PyArrow conversion.
