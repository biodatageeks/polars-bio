...
- [ ] 1.5 Add one CI job that installs `bioconda::hmmer` and runs the live `esl-alistat`/`esl-reformat` assertions; mark those tests `skipif(shutil.which("esl-alistat") is None)` so every other job skips them cleanly.
