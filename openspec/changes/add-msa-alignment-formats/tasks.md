...
- [ ] 1.3 Check in `tests/data/msa/generate_fixtures.py` (pyhmmer) that produces: `PF00001_hmmalign.sto` (`#=GR PP`, `#=GC PP_cons`/`RF`, 200-column blocks), `multi.sto` (PF00001 + RF00001 in one file), `query_dotted.a2m` (Easel-reconstructed rectangular rows), `hdr.a3m` (`#A3M#` + `#` lines prepended to `query.a3m`), and hand-written minimal edge files: missing trailing `//`, wrong header, empty file, single-record file, `.gz`/`.bgz` variants.
- [ ] 1.4 Add `pyhmmer` and `biopython` to `[project.optional-dependencies]` next to `pysam`/`pyBigWig`, and to the CI test extras; parity tests `pytest.importorskip` both.
- [ ] 1.5 Write the ~15-line reference line-splitter used as the differential oracle for raw `gs`/`gr` bags and `describe_sto` (no block handling, no streaming — intentionally naive), and assert it agrees with pyhmmer on names/sequences for the single-block fixture so the oracle itself is cross-checked once.
