...
| Duplication level | ✅ exact | ✅ ≈exact (0.005 pts) | ❌ **+6.16 pts total dedup** |
| K-mer content | ✅ same top-20; Δobs/exp 0.0019 | ⚠ top k-mer correct; list differs | ❌ **wrong top k-mer** |

Per-sequence quality is the worst: RastQC invents a Q40 bin FastQC never emits and drops **671,121 reads** into it — a mean-quality rounding bug. %GC is off by one because it rounds where FastQC truncates. But the deepest tell is in the last two rows: **RastQC's output changes with the thread count.** At `-t 1`, its FastQC-comparable duplication percentages match within rounding, but the k-mer top-20 list already differs. At `-t 4`, the duplication estimate itself drifts (+6.16 points total dedup) and even the top k-mer is wrong — its sampling runs per-worker, so the "answer" depends on how many cores you gave it. polars-bio is value-identical across the checked partition counts and matches FastQC 0.12.1 under the stated exact/tolerance parity checks. (The full 12-module drift table is in the [benchmark post](fastqc-benchmark-2026-07.md#correct-at-every-thread-count).)
...
Specify first. rewrites.bio's planning phase — Think Big, then Work Small: *"Start with the simplest function that produces testable output. Validate it. Then extend."* Pin the equivalence contract before writing a line: which modules, which FastQC version, what tolerance. Then test first. For every module we write the FastQC assertion *before* the implementation, watch it fail against real golden output, and implement until it goes green. polars-bio's FastQC has dedicated golden and arithmetic tests across the 12-module suite — value assertions, not header greps — plus a partition-invariance check that runs the same input at multiple partition counts and demands identical parsed values. On **real data** ([§4.1](https://rewrites.bio/#test-and-benchmark)), against a **pinned** FastQC 0.12.1 ([§4.3](https://rewrites.bio/#pin-versions)). That is why the drift table above has a column of green checks.
