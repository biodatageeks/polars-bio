...
use datafusion::common::{exec_err, DFSchema, ScalarValue, TableReference};
...
use datafusion::prelude::{CsvReadOptions, DataFrame, JsonReadOptions, ParquetReadOptions};
...
impl PySessionContext {
    ...
    pub fn register_avro(
        ...
        let _ = (name, path, schema, file_extension, table_partition_cols, py);
        Err(PyRuntimeError::new_err(
            "Avro support is disabled in the polars-bio DataFusion Python build",
        )
        .into())
    }
    ...
    pub fn read_avro(
        ...
        let _ = (path, schema, table_partition_cols, file_extension, py);
        Err(PyRuntimeError::new_err(
            "Avro support is disabled in the polars-bio DataFusion Python build",
        )
        .into())
