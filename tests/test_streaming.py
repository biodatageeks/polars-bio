...
class TestStreaming:
    ...
    @pytest.mark.skip(reason="Polars 1.32.2 has a bug with engine='streaming' - with_optimizations was removed")
    def test_overlap_plan(self):
        plan = str(self.result_overlap_stream.explain(engine="streaming"))
        assert "streaming" in plan.lower()

    @pytest.mark.skip(reason="Polars 1.32.2 has a bug with engine='streaming' - with_optimizations was removed")
    def test_nearest_plan(self):
        plan = str(self.result_nearest_stream.explain(engine="streaming"))
        assert "streaming" in plan.lower()

    @pytest.mark.skip(reason="Polars 1.32.2 has a bug with engine='streaming' - with_optimizations was removed")
    def test_count_overlaps_plan(self):
        plan = str(self.result_count_overlaps_stream.explain(engine="streaming"))
        assert "streaming" in plan.lower()

    @pytest.mark.skip(reason="Polars 1.32.2 has a bug with engine='streaming' - with_optimizations was removed")
    def test_coverage_plan(self):
        plan = str(self.result_coverage_stream.explain(engine="streaming"))
        assert "streaming" in plan.lower()
    ...
    def test_overlap_execute(self):
        ...
        assert len(result.collect(engine="streaming")) == len(PL_DF_OVERLAP)
    ...
    def test_nearest_execute(self):
        ...
        assert len(result.collect(engine="streaming")) == len(PL_DF_NEAREST)
        result.sink_csv(file)
        expected = pl.read_csv(file)
    ...
    def test_count_overlaps_execute(self):
        ...
        assert len(result.collect(engine="streaming")) == len(PL_DF_COUNT_OVERLAPS)
        result.sink_csv(file)
        expected = pl.read_csv(file)
    ...
    def test_coverage_execute(self):
        ...
        assert len(result.collect(engine="streaming")) == len(self.result_coverage_bio)
        result.sink_csv(file)
