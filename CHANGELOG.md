...
- `concurrent_fetches` now defaults to `8` in every `read_*`, `scan_*`,
  `describe_*` and `register_*` function (33 signatures were still at `1`;
  ...
  comment). Pass `concurrent_fetches=1` to keep a single request, e.g. for
  pre-signed GET-only URLs. S3 needs the datafusion-bio-formats#253 pin to
  benefit.
